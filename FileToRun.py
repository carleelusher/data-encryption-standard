...
def process_input_text(filename: str) -> list:
    ...
    blocks = [[] for block in range(len(bits)//64)]
    for i in bits:
        blocks[bits.index(i)//64].append(i)

    return blocks


def process_input_password(password: str) -> list:
    """
...
# process_input_password('password')

process_input_text('plaintext.txt')